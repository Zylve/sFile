    ...
    public:
        ...
        scriptBuilder(std::string _file, bool _editing) {
            ...
            if(!_editing && !std::filesystem::exists(file)) {
                ...
                std::cout << "Created new script file: " << file << "\n";
        ...
        scriptList(std::string _path) {
            for (const auto & file : std::filesystem::directory_iterator(_path)) {
                if (file.path().extension() == ".sh") {
                    // I am not changing this, cry about it.
                    std::cout << std::string(file.path()).substr(std::string(file.path()).find_last_of("/\\") + 1) << "\n";
                }
            }
        }
};